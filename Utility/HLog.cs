...
namespace AISIN_WFA.Utility
{
    public static class HLog
    {
        ...
        public static void log(string msgType, string logMsg)
        {
            if (logMutex.WaitOne(mutexWait))
            {
                ...
                {
                    WriteApplicationEventLog(string.Format("HLog.log exception={0} msg={1}", ex.Message, string.Format("{0}", logMsg)));
                }
                logMutex.ReleaseMutex();
            }
            else
            {
                WriteApplicationEventLog(string.Format("HLog.log mutex timeout msg={0}", string.Format("{0}", logMsg)));
            }
        }

        public static void log(eLog msgType, string logMsg)
        {
            if (logMutex.WaitOne(mutexWait))
            {
                try
